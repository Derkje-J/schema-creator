...
SchemaForm.prototype.insertFormSection = function( type, embed ) {
	...
	for( var i in schema_fields ) {
		...
		if ( schema_field_property_type.length == 2 && 
			...
			output += '" value="' +  htmlEscape( field_url.val() ) + '"]' + field_text.val() + '[/scprop]';
		...
		// Meta content
			...
			output += '" content="' + htmlEscape(  schema_field_meta.val() ) + '"]' + schema_field_display.val() + '[/scmeta]';
		...
		// Other props
			...
			for ( var j in schema_field_property_type ) {
				...
				output += '" value="' + htmlEscape( schema_field_value.val() ) + '"/]';
			}
		}
	}	
