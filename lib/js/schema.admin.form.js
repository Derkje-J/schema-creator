...
SchemaForm.prototype.insertFormSection = function( type, embed ) {
	...
	for( var i in schema_fields ) {
		...
		switch( schema_field_property ) {
			case 'additionalType' :	
				schema_field_property_type.unshift( 'sc_meta_~override~' );
			break;
		}
		...
		// Meta content
			...
			// OVERRIDE
			if ( schema_field_property_type[0] == 'sc_meta_~override~' ) {
				
				var schema_field_value = jQuery( "[name='" + schema_id + "\[" + schema_field_property + "\]\[" + schema_field_property_i + "\]\[" + schema_field_property_type[1] + "\]']");
		...
		// Other props
			...
			for ( var j in schema_field_property_type ) {
				...
				output += '" value="' + htmlEscape( schema_field_value.val() ) + '"/]<br>';
			}
		}
	}	
