...
SchemaForm.prototype.insertFormSection = function( type, embed ) {
	...
	for( var i in schema_fields ) {
		...
		if ( schema_field_property_type.length == 2 && 
			...
			output += '" value="' +  htmlEscape( field_url.val() ) + '"]' + field_text.val() + '[/scprop]<br>';
		...
		// Arbitrary embed
			...
			// Update embed type. Might be more specific
			if ( field_embed.val() !== undefined && field_embed.val() !== '' ) {
				embed_type = jQuery( '#schema_' + field_embed.val() + '_type' ).val();
			}
		...
		// Meta content
			...
			if ( do_meta === true ) {
				...
				var schema_field_value = jQuery( "[name='" + schema_id + "\[" + schema_field_property + "\]\[" + schema_field_property_i + "\]\[" + schema_field_property_type[0] + "\]']");
				
				if ( schema_field_value.val() === undefined )
					schema_field_value.val('');
					
				output += '[scmeta prop="' + schema_field_property + '" range="' + schema_field_property_type[1] ;
				output += '" content="' + htmlEscape( schema_field_value.val() ) + '"/]<br>';				
		 	...
		 	// DEFAULT
				...
				output += '[scmeta prop="' + schema_field_property + '" range="' + real_type[2] ;
				output += '" content="' + htmlEscape(  schema_field_meta.val() ) + '"]' + schema_field_display.val() + '[/scmeta]<br>';
			}
		...
		// Other props
			...
			for ( var j in schema_field_property_type ) {
				...
				switch( schema_field_property ) {
					...
					case 'text' :
					case 'instructions':
						if ( schema_field_property_type[j] == 'sc_Text' )
							schema_field_property_type[j] = 'sc_Paragraph';
					break;
					...
					case 'photos':
						// sc_Image ?
						if ( schema_field_property_type[j] == 'sc_Url' )
							schema_field_property_type[j] = 'sc_Image';
					break;
				}
				
				if ( schema_field_property_type[j] == 'sc_Paragraph' ) {
					output += '[scprop prop="' + schema_field_property + '" range="' + schema_field_property_type[j] ;
					output += '"]' + schema_field_value.val() + '[/scprop]<br>';
				} else {
					output += '[scprop prop="' + schema_field_property + '" range="' + schema_field_property_type[j] ;
					output += '" value="' + htmlEscape( schema_field_value.val() ) + '" ' + attribute_defaults +'/]<br>';
				}
			}
		}
	}	
