					...
					function( result ) { 
						...
						for ( var inherited_type in result.properties ) {
							...
							for ( var i in result.properties[ inherited_type ] )
								html += schema_build_property( inherited_type, schema_embed, result.properties[ inherited_type ][i] );
...
//////////////////////////////////////////////////////////
	...
	var schema_build_property = function( schema_type, schema_type_i, json_type ) {
		
		var property_id = schema_type + '_' + schema_type_i + '_' + json_type.id + '_0';
		var property_name = 'schema[' + schema_type_i + '][' + json_type.id + '][0]';
	...
	var schema_build_property_field = function( hidden_id, hidden_name, option_range, classes ) {
		...
		if ( type === 'sc_Boolean' )
				return { 'sc_Boolean': '<input type="checkbox" style="' + style + '"  id="' + hidden_id + '_' + type + '" ' +
					...
					'value="' + schema_get_current( hidden_id + '_' + type ) + '" style="display:none;" class="' + classes + ' form_eight' + '"/>'
				};
